...
test("bridge contract is user-scoped, read-only, and program-state aware", async () => {
  ...
  assert.match(server, /function registerTools\(server, supabase, jwtClaims\)/);
  assert.match(server, /registerTools\(server, supabase, jwtClaims\)/);
  assert.doesNotMatch(server, /context\.jwtClaims|context\.supabase/);
  const draftStart = server.indexOf('"create_coach_update_draft"');
  const submitStart = server.indexOf('"submit_coach_update"', draftStart);
  ...
  assert.ok(weightMigration.includes("add column if not exists weight_entries"));
