-- Preserve a Coach-selected program start as a date-only local calendar value.
-- PostgreSQL DATE deliberately avoids UTC timestamp shifts for Friday/Saturday starts.

alter table public.coaching_update_requests
  add column if not exists program_effective_date date;
...
begin
  if (select auth.jwt() ->> 'client_id') is not null then
    if old.user_id is distinct from new.user_id
      ...
      or old.program_effective_date is distinct from new.program_effective_date
