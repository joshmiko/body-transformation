-- Reviewed coach weigh-ins are first-class, top-level coach update data.
-- Apply before deploying the app/Edge Function changes in this PR.

alter table public.coaching_update_requests
  add column if not exists weight_entries jsonb;
...
begin
  if not exists (
    select 1
    from pg_constraint
    where conname = 'coaching_update_requests_weight_entries_check'
      and conrelid = 'public.coaching_update_requests'::regclass
  ) then
    alter table public.coaching_update_requests
      add constraint coaching_update_requests_weight_entries_check
      check (
        weight_entries is null
        or (
          jsonb_typeof(weight_entries) = 'array'
          and jsonb_array_length(weight_entries) <= 14
        )
      );
  end if;
end
...
begin
  if (select auth.jwt() ->> 'client_id') is not null then
    if old.user_id is distinct from new.user_id
      ...
      or old.weight_entries is distinct from new.weight_entries
